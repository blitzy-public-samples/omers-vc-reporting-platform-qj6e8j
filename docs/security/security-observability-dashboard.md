...
- **Signal source:** the `token_validation_failure` record emitted when a token is rejected at a service's authentication boundary (the authentication service's protected route, `main.py`; and the API gateway's token dependency in `main.py` and its authentication middleware in `app/__init__.py`). The underlying `validate_token` decode enforces `algorithms=["HS256"]`; this panel makes those rejections observable. Note: because the gateway's token dependency re-decodes the bearer token through a pre-existing, out-of-scope code path (see `SECURITY.md` → Known Issues), gateway `token_validation_failure` records can also fire for otherwise-valid tokens; treat gateway counts as a superset until that path is repaired.
