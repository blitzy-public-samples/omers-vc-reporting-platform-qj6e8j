...
- The reporting-financials application imports and starts cleanly: `app/routers/financials.py` reads the settings **instance** (`config.DATABASE_URL`), not the class attribute, so `main.create_app` and the module-level `app` build successfully (verified at runtime — `uvicorn src.backend.reporting_financials_service.main:app` serves and `/docs` returns 200). One residual, pre-existing non-security check remains: the full-application test (`tests/test_financials.py`) does not collect because it imports a `financials_router` symbol the router module does not export (the router is named `router`) — a broken-test defect unrelated to this security remediation and tracked separately (AAP §0.8.2). Its security regression suites (`tests/test_security_config.py`, `tests/test_security_cors.py`) pass, because they exercise the `Config` settings class and the CORS application factory directly.
- The reporting-metrics application imports and starts cleanly: `app/models/models.py` imports `UUID` from `sqlalchemy.dialects.postgresql` (the correct location, which SQLAlchemy 1.4.x exposes), so `main.app` builds successfully (verified at runtime — `uvicorn src.backend.reporting_metrics_service.main:app` serves and `/docs` and `/health` return 200). Its full-application test (`tests/test_metrics.py`) collects and its `tests/test_security_config.py` suite passes; the previously noted top-level-`UUID` import blocker is cleared (the only caveat is that `test_metrics.py`'s async cases skip when no async plugin is installed).
...
- **API-gateway authentication contract (partially remediated; residual in a non-deployed variant).** The deployed gateway entrypoint — the module-level `app = create_app()` in `api_gateway/main.py` — now extracts the JWT from the `Authorization: Bearer` header via a single `OAuth2PasswordBearer` dependency and validates it exactly once. This engagement changed it (commit `655a630`) from the prior pattern that bound the raw token as a query dependency and then validated it twice, which reduces the CWE-598 token-in-URL exposure (URLs, server access logs, browser history) and lets `/protected` return 200 for a valid bearer (rationale in the [security decision log](docs/security/decision-log.md)). The residual double-validate / query-dependency pattern — together with the request middleware that indexes `Authorization.split(" ")[1]` (so a header without a space raises and surfaces as a 500) — remains only in the **non-deployed** alternate application factory `api_gateway/app/__init__.py` (F-APPVAR-1), which is not the ASGI entrypoint. The security-event logging added by this engagement wraps the existing flow without changing its decode count or status semantics. Unifying that alternate factory with `main.create_app()` and adding full `Authorization` header-shape validation is the broader architectural authentication rework the AAP defers (§0.8), and is out of scope here.
...
- **Alternate auth application factory swallows a router import failure (MAJOR, pre-existing).** The authentication service's `app/__init__.py` builds a module-level app that wraps its router import in `try/except ImportError` and only prints a message, so a genuine router failure would yield a docs-only app with empty `on_event` startup/shutdown handlers. This `try/except`-and-`print` pattern and the empty lifecycle handlers are pre-existing (present verbatim at the committed baseline); the security-remediation diff to this file is limited to the AAP-authorized CORS allow-list wiring and correlation-id/CORS security-event logging (§0.5.1–0.5.2), and does not change the import-failure semantics. The auth app is produced by the `create_app()` factory and has no module-level `app` (it is defined only under `if __name__ == "__main__"`), so the correct ASGI invocation is `uvicorn --factory src.backend.authentication_service.main:create_app` (run from the repository root). The shipped Dockerfile `CMD ["uvicorn", "main:app", ...]` therefore does not load as-built (`Error loading ASGI app. Attribute "app" not found in module "main"`) — a pre-existing, out-of-scope entrypoint/packaging deployability gap (the Dockerfile CMD is unmodified by this engagement), consistent with the build-context note above. `create_app()` itself includes the routers directly and is independent of this alternate factory. Making the alternate factory fail-closed or equivalent to `main.create_app()` is a functional/startup-behavior change outside the five in-scope security layers and the Minimal Change Clause (AAP §0.8) — follow-up.
...
- **`data_transformation` function runtime alignment (resolved).** The function manifest pins patched `requests` / `urllib3` that require Python `>= 3.10`; the function app's deploy runtime in `infrastructure/terraform/main.tf` is `PYTHON|3.11`, which already satisfies that requirement, so these pins install cleanly at deploy time. No runtime-alignment follow-up remains.
